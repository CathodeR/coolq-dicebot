...
bool dicebot::binary_tree_split_dice(std::string const& str_input, std::string& str_roll_command, std::string& str_roll_detail,
    ...
    if (!oper_target.tail.empty() || !source.empty()) {
        ...
        if (!t_message.empty()) {
            ...
            if (p == std::string::npos) {
                ...
                message.assign(t_message.substr(p));
        }
    }
    ...
    if (oper_cal_successfule) {
        ...
        if (oper_target.no_dice)
            str_roll_detail.assign("");
        else
            str_roll_detail.assign(oper_target.str_roll_detail);
    } else {
...
bool operation_item::calculate() {
    ...
    case operation_mode::NUMBER: {
        ...
        if (exist_percent)
            str_leave_percent = this->mode_num_like_source.substr(0, pos_of_percent);
        else
            str_leave_percent = this->mode_num_like_source;

        this->ret_value = str_leave_percent;
        if (exist_percent) this->ret_value = this->ret_value * 0.01f;
    ...
    case operation_mode::OPERATOR: {
        if (this->is_unary_operator) {
            ...
            if (this->second != nullptr) right_available = this->second->calculate();
            ...
            bool right_available = false;
            if (this->first != nullptr) left_available = this->first->calculate();
...
bool operation_item::fail() { return false; }
    ...
    if (!this->left_bracket_location->empty()) {
        ...
        while (this->list_output->size() > pos_of_ire_blacket) {
            if (this->list_output->back()->is_num_like())
                this->i_num_like_count--;
            else if (this->list_output->back()->is_operation() && !this->list_output->back()->is_unary_operator)
