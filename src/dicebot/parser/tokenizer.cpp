...
tokenizer::tokenizer(std::deque<token_t>& tokens, tokenizer_flag const& flag,
...
std::string tokenizer::tail(token_t const& target) const {
    ...
    if (target.source_index > 0) {
        size_t next_begin =
            this->sources_sites->at(target.source_index).macro_end;
...
token_t* tokenizer::cur_token() const {
...
token_t* tokenizer::next_token() const {
    ...
    temp_token.pos_next = this->sources->at(temp_token.source_index)
                              ->find_first_not_of(' ', temp_token.pos_next);

    if (temp_token.pos_next == npos
        || temp_token.pos_next
               >= this->sources->at(temp_token.source_index)->size()) {
        if (temp_token.source_index == 0) {
            ...
            temp_token.pos_next =
                (*this->sources_sites)[temp_token.source_index].macro_end;
            temp_token.source_index = 0;
            temp_token.pos_next =
                this->sources->at(temp_token.source_index)
                    ->find_first_not_of(' ', temp_token.pos_next);
            if (temp_token.pos_next == npos) {
                temp_token = {token_index::index_stop, 0, npos, npos};
                this->token_container.emplace_back(std::move(temp_token));
    ...
    bool has_got =
        this->get_punctuator(temp_token) || this->get_number(temp_token);
    ...
    if (this->do_parse_identifier // parse macro only when parse identifier is
        ...
        && (this->sources_sites->back().macro_end
                == 0 // only the first one (for origin source) have a macro_end
                     // of 0
            || this->sources_sites->back().macro_end
                   < temp_token.pos_next)) // last one is not a macro
    {
        has_got = this->resolve_identifier(temp_token);

        if (has_got && temp_token.id == token_index::index_macro) {
            this->sources_sites->push_back(
                {temp_token.pos_cur, temp_token.pos_next});
            const std::string* expand_macro =
                &macro_map->find(this->token_string(temp_token))->second;
            this->sources->push_back(expand_macro);
            temp_token = {
                token_index::index_begin, this->sources->size() - 1, 0, 0};
            this->token_container.emplace_back(std::move(temp_token));
...
std::pair<size_t, bool> greed_map_find(std::string const& source,
                                       size_t start_pos,
                                       tokenizer::macro_map_t const& map) {
    ...
    std::copy(
        source.cbegin() + start_pos, end_point, std::back_inserter(source_cp));
...
bool tokenizer::resolve_identifier(token_t& target) const {
    ...
    if (find_result.second) {
        ...
        std::string suppose_macro =
            src.substr(target.pos_next, find_result.first);
        ...
        do {
            ...
            if (suppose_target.pos_next
                >= this->sources->at(suppose_target.source_index)->size())
            ...
            if ((this->ambi_flag.ambiguity_d) && suppose_macro == "d") {
                if (!(this->token_container.empty())
                    && this->token_container.back().id
                           == token_index::keyword_d)
                    break;
                token_t tn = peek_next(suppose_target);
                if (tn.id == token_index::punct_lbrace
                    || tn.id == token_index::punct_lparenthesis
                    || tn.id == token_index::index_number
                    || tn.id == token_index::ambiguity_indentifier)
                    return false;

                if (tn.id == token_index::index_stop
                    || tn.id == token_index::punct_rbrace
                    || tn.id == token_index::punct_rparenthesis
                ...
                if (tn.id == token_index::ambiguity_macro_d) {
                    ...
                    if (tnn.id == token_index::ambiguity_macro_d
                        || tnn.id == token_index::index_number)
                        break;
                    else
                        return false;
                }
