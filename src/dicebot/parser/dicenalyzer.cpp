...
static auto build_rdk = [](dice_rdk_mode mode, const syntax_nterminal *p_syntax_item) -> auto {
    ...
    case dice_rdk_mode::single_d: {
        auto p1 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[1].get()));
        ret->face = std::move(p1->what);
        break;
    }
    case dice_rdk_mode::numbered_d: {
        auto p1 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[0].get()));
        auto p2 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[2].get()));
    ...
    case dice_rdk_mode::numbered_d_kl: {
        auto p1 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[0].get()));
        auto p2 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[2].get()));
        auto p3 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[4].get()));
        ret->dice = std::move(p1->what);
        ret->face = std::move(p2->what);
...
p_component diceparser::build_component_from_syntax(const syntax_item *root) {
    if (root->is_terminal()) {
        ...
        case production_type::dicelet_u_const_unit_sharp_rand_unit: {
            ...
            auto p1 = std::dynamic_pointer_cast<comp_number>(build_component_from_syntax(p_syntax_item->items[0].get()));
            ret->count = p1->what;
            ret->child = build_component_from_syntax(p_syntax_item->items[2].get());
            return ret;
        }
...
void comp_dice_rdk::print(str_container &strlist) const noexcept {
    ...
    case dice_rdk_mode::numbered_d:
        ...
        break;
    case dice_rdk_mode::numbered_d_k:
        strss << this->dice.str_holder() << "d" << this->face.str_holder() << "k" << this->keep.str_holder();
        break;
    case dice_rdk_mode::numbered_d_kl:
