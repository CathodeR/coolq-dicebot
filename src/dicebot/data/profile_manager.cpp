...
static bool read_database(user_profile &profile, int64_t const user_id) {
    ...
    return db_manager::get_instance()->exec(
        ostrs_sql_command.str().c_str(), &profile, [](void *data, int argc, char **argv, char **azColName) -> int {
            ...
            bool good_query = profile->sys_vars.decode(argv[0]) && profile->def_roll.decode(argv[1]) && profile->mac_rolls.decode(argv[2]);
            if (good_query)
                return SQLITE_OK;
            else
                return SQLITE_ABORT;
...
static bool exist_database(int64_t const user_id) {
    ...
    bool ret = false;
    db_manager::get_instance()->exec(ostrs_sql_command.str().c_str(), &ret, [](void *data, int argc, char **argv, char **azColName) -> int {
...
const profile_manager *profile_manager::get_instance() noexcept { return instance.get(); }
...
sptr_user_profile profile_manager::get_profile(int64_t const user_id) const {
    auto iter = this->profiles->find(user_id);
    if (iter != this->profiles->end()) {
        return iter->second;
    } else {
