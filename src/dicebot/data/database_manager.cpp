...
database_manager* database_manager::create_instance(
    const char* str_app_dir) noexcept {
...
database_manager* database_manager::get_instance() noexcept {
    return instance.get();
}
...
int database_manager::register_table(const char* str_table_name,
                                     const char* str_table_sql) {
...
inline int database_manager::is_table_exist(const char* table_name,
                                            bool& is_exist) {
    is_exist = false;
    if (is_no_sql_mode) return SQLITE_ABORT;
