...
database_manager* database_manager::get_instance(std::string str_app_dir) {
    if (database_manager::instance == nullptr) database_manager::instance = new database_manager(str_app_dir);
    return database_manager::instance;
}

database_manager* database_manager::get_instance() { return database_manager::instance; }
...
int database_manager::register_table(std::string str_table_name, std::string str_table_sql) {
    ...
    if (i_ret_code == SQLITE_OK && !isExist) {
        ...
        int i_ret_code_2 =
            sqlite3_exec(this->database, sql_command, &sqlite3_callback_general, (void*)&database::oper_create, &pchar_err_message);
