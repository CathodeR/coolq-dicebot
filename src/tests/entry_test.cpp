...
TEST_F(entry_test, roll_wod) {
    ...
    std::regex reg_wodo6(u8"^ \\* dice 掷骰: oWoD = \\[(\\d{1,2}\\*? \\+ ){5}\\d{1,2}\\*?\\] = \\d");
    std::regex reg_wodn6(u8"^ \\* dice 掷骰: nWoD = \\[(\\d{1,2}\\*? \\+ ){5,}\\d{1,2}\\*?\\] = \\d");

    std::regex reg_wodo6_msg(u8"^ \\* dice test 掷骰: oWoD = \\[(\\d{1,2}\\*? \\+ ){5}\\d{1,2}\\*?\\] = \\d");
    std::regex reg_wodn6_msg(u8"^ \\* dice test 掷骰: nWoD = \\[(\\d{1,2}\\*? \\+ ){5,}\\d{1,2}\\*?\\] = \\d");
    this->base_call(ei, ".ndice");
...
TEST_F(entry_test, roll_fate) {
    ...
    std::regex reg_fate1_msg(u8"^ \\* dice test 掷骰: FATE = \\[([o+\\-] ){3}[o+\\-]\\] \\+ 1 = -?\\d");
